use yew::prelude::*;
use yew::worker::{
    Agent, AgentLink, HandlerId, Public,
};
...
impl Component for Model {
    ...
    fn create(_props: Self::Properties, link: ComponentLink<Self>) -> Self {
        let worker = Worker::bridge(
            link.callback(|worker_output| Self::Message::WorkerMessage(worker_output))
        );
    ...
    fn view(&self) -> Html {
        html! {
            <>
                ...
                <button onclick=self.link.callback(|_| Message::RunWorker)> { "submit" } </button>
                <br /> <br />
                <h3> { "Main thread value: " } { self.clicker_value } </h3>
                <button onclick=self.link.callback(|_| Message::Click)> { "click!" } </button>
            </>
        }
    }
}
